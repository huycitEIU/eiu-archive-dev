...
import { findUserByUsername } from "../../src/repositories/userRepository.js";
...
describe("DELETE /api/auth/:userId", () => {
  it("should return 200 and a success message for valid user deletion", async () => {
    const userToDelete = await findUserByUsername(testUser.username);
    ...
    const response = await request(app)
      .delete(`/api/auth/${userIdToDelete}`);
  ...
  it("should return 404 for non-existing user", async () => {
    ...
    const response = await request(app)
      .delete(`/api/auth/${nonExistingUserId}`);
